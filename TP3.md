...
```
Réponse :
Il faut activer le chiffrement des secrets au niveau de la couche applicative (Application-layer secret encryption) dans la configuration du cluster GKE en utilisant une clé Cloud KMS. C'est indispensable en production pour s'assurer que même si un attaquant accède à la base de données etcd sous-jacente du cluster ou à ses disques de stockage, il ne puisse pas lire en clair les mots de passe, tokens ou clés d'API (l'encodage base64 par défaut n'offrant aucune sécurité).
```
...
- name: api-gateway
  image: europe-west9-docker.pkg.dev/[PROJECT_ID]/tp2-registry/tp2-app:v
  ports:
- containerPort: 8080
...
```
Réponse :
Il maintiendra 4 réplicas en régime de pointe.
Explication : Pour traiter 80 événements/seconde tout en maintenant l'utilisation CPU autour de sa cible de 60%, il faut diviser la charge totale par la capacité d'un pod à ce seuil (80 événements / 20 événements par pod = 4 pods). Comme 4 est compris entre minReplicas (2) et maxReplicas (10), le HPA stabilisera le nombre de réplicas à 4.
```
...
longitude: base.lng + (Math.random() - 0.5) _ 0.1,
speed_kmh: Math.floor(Math.random() _ 40 ) + 70 , // 70-110 km/h
fuel*level: Math.floor(Math.random() * 60 ) + 20 , // 20-80%
...
```
Réponse :
Kafka distribue les 6 partitions du topic truck-positions de manière équitable entre les 3 instances du Consumer Group (2 partitions par instance). Si une 4ème instance est ajoutée, Kafka effectue un rebalancing pour redistribuer les partitions (par exemple, 2 instances avec 2 partitions et 2 instances avec 1 partition, ou une répartition similaire selon la stratégie de partitionnement). Cela permet de scaler horizontalement le traitement.
...
```
Réponse :
On peut utiliser des outils comme **Datree**, **Checkov**, ou **Kube-score**. Ces outils permettent de définir des politiques de sécurité et de conformité (Policies as Code) et de valider les manifests YAML avant le déploiement pour s'assurer qu'ils respectent les bonnes pratiques (limits, probes, non-root user, etc.).
...
```
Réponse :
1. **Augmenter le nombre de réplicas** du tracker-consumer (scaling horizontal) jusqu'à 6 (le nombre de partitions) pour paralléliser davantage le traitement.
2. **Optimiser le code du consumer** (ex: traitement asynchrone plus performant, optimisation des requêtes en base de données).
3. **Augmenter les ressources (CPU/RAM)** des pods existants (scaling vertical) si le retard est dû à une saturation des ressources.
